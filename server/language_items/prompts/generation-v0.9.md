# Multilingual A1 Item Generation Prompt v0.9

Generate candidate-visible content in lockedConstraints.language, its proposed scoring package, and separate English translations for the author and reviewer, using the locked Item Generation Package supplied by the server.

Language requirements:

- The same Item rules apply to Chinese, English and Spanish. Interpret legacy references to Chinese script, Simplified Chinese, pinyin or typedChinese as the selected language and its ordinary orthographic conventions for English/Spanish. Preserve semantic Can-do, task, scoring and difficulty requirements and machine IDs. Do not require Han characters or pinyin in an English/Spanish item, or copy Chinese wording from shared defaults. This adaptation does not claim empirical calibration.
- For English items return englishTranslations: [] because the authored text is already English. For Chinese and Spanish items supply the separate English references described below. English vocabulary may have no Meaning; use its form and other supplied metadata without inventing a compulsory meaning requirement.

- Use the locked language: zh = Simplified Chinese, en = English, es = Spanish, suitable for an A1 learner of that language. All candidate-visible content, proposed answers, source exercise data and source language metadata must match it. Candidates must never see author translations or internal implementation terms.
...
- Each translation has `path` (an absolute RFC6901 JSON pointer relative to candidatePayload), `sourceText` (the exact unchanged source string), and `englishText` (a faithful, nonempty English translation containing no Chinese text). Escape `~` as `~0` and `/` as `~1` in path segments. Use array positions, not object IDs.
...
When taskBrief.reviewPlan is supplied, use its fixed checks and saved custom review criteria to construct assessable content with observable evidence. Custom criteria supplement the locked rules and must never waive them. Do not insert checklist text, assessment explanations, pass claims, rule identifiers or internal evidence into candidate content. A separate review evaluates the generated draft; generation must not pre-approve its own work.
