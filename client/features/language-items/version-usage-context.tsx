...
import { optionLabel, DIFFICULTY_LABELS, ITEM_FORMAT_LABELS, slotLabel, WORKBENCH_LABELS } from "./labels";
...
export function VersionUsageContext({ version }: { version: LanguageItemVersion }) {
  ...
  const fields = [
    [WORKBENCH_LABELS.blueprintSlot, slotLabel(package_.blueprintSlotId, registry.data, package_.itemFormatId)],
    [WORKBENCH_LABELS.itemFormat, ITEM_FORMAT_LABELS[package_.itemFormatId] ?? package_.itemFormatId],
    ...
    [WORKBENCH_LABELS.context, context ? registryDisplayText(context.label) : package_.content.contextId],
  ...
  return <Box as="details" borderWidth="1px" borderRadius="md" p={3}>
    <Text as="summary" cursor="pointer" fontSize="sm" fontWeight="medium">Version {version.versionNumber} · saved {new Date(version.createdAt).toLocaleString()}</Text>
    <Stack mt={3} gap={2}>
