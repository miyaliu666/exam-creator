...
export function exerciseReviewSchemas(registry, type) {
  ...
  task.properties.blueprintSlotId = { enum: capabilities.map(row => row.blueprintSlotId) };
...
export function validateExerciseCandidate(pkg, registry) {
  ...
  const rule = registry.exerciseTemplateRules?.find(entry => `exercise:${entry.id}` === pkg.blueprintSlotId && entry.exerciseType === type && entry.primaryCanDoId === pkg.content.primaryCanDoId);
