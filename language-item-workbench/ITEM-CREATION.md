...
**Item** 是一份完整题目，可以包含多个作答字段或对话轮次。**Item rules** 是 Assessment Settings 中 Primary Can-do × Item format 配置组合及其可复用中央规则；**Item setup** 是本题的五项设置，在 Can-do 与题型上增加 Domain、Context、Difficulty。**AI draft** 是某道题的一份 AI 备选题稿；**Candidate** 始终指考生。工作台中的 **Question** 仅指题干文本；旧 Exams、Attempts、Exam Metrics 和 User Management 保留原有术语、界面和业务逻辑。

1. **New items：选语言、设置和数量。** 打开 `/language-items/new`，先选 Chinese、English 或 Spanish，再选择 Primary Can-do、Item format、Domain、Context、Difficulty。三种语言共用当前 Assessment Settings 中的 Can-do、Context、难度和评分规则；所选语言决定目标内容库、生成语言和检查范围。每次新建共用一套设置，Number of items 默认是 1，改成多个即为多题创建；需要不同设置时，提交当前组后再进入 New items。未提交的设置、数量和目标在当前浏览器会话中保留；旧的多组计划仍可编辑并完成，但不能再追加设置，也不会被截断或改成默认数量。
2. **选择语言目标并生成。** 单题选择 What this item should assess；多题使用 Targets required in every item，并可增加 Different targets for different items。展开 Preview each item's targets 可查看每题分配。AI drafts per item 与 Number of items 并列显示，是必填数量，默认 1；新计划按 Item rule 自动命名，无需填写名称。点击 Generate 1 item / Generate N items 后进入 `/language-items/batches?batchId=...` 查看任务。
3. **Generation jobs：查看进度并打开题目。** 批次按名称和创建时间区分；每组显示一次五项设置与共同必选目标，每题显示序号、额外分配目标和生成状态。批次和题目的内部 ID 不在页面展示，数据关联与打开题目仍使用原 ID。页面保留 Pause / Continue、历史和 Open item；离开页面不停止生成，Pause 等待正在进行的模型调用结束。Open item 打开 Prepare，已有候选优先显示；Use this draft 采纳后进入编辑。生成状态与分配目标不表示人工审核通过或正文实际使用了这些目标。
...
**Language coverage** 位于 `/language-items/coverage`。Overview 和 Find items 分别选择统计语言；语言选择限定内容目录、题目库存、组合统计和补题建议，不改变共享出题规则。明确设置库存目标后，可把携带所选语言的兼容补题建议带到 New items；建议通过当前浏览器会话交接，由出题人确认数量和目标后再生成。Use suggested setup 打开设置窗口，Apply suggestion 才会应用建议；已有一组设置时会明确替换该组设置、数量和目标，不追加第二组。旧的多组计划仅在出题人主动移除至一组后才能应用建议；也可先完成旧计划，再从 Language coverage 重新获取建议。

Language coverage 统计已保存的核心语言目标，并区分最新批准版本与草稿／审核中内容。Overview 默认显示完整语言目录；选择命题条件后，Total 和列表共同限定为这些条件下可用的语言条目，保留尚未命题的条目。互不兼容的命题条件显示无匹配组合。Category 和 Search 在这个范围内继续筛选；Find items 按目标检索题目。作者观察记录不决定这两个库存计数，背景材料不计入核心目标覆盖。
...
创建后五项选择集中显示在一处 Item setup 摘要中，不需要再次填写。需要纠正 Domain、Context 或 Difficulty 时，点击 Edit item setup。Primary Can-do、Item format 和绑定的中央规则版本保持固定；需要改变这些身份条件时，应新建 Item。

五个下拉框采用相同交互：占位提示只在未选择的输入框内显示，不进入选项列表。展开时其他字段淡化，当前选中项带勾选；选择、按 Esc 或点击列表外后恢复正常显示，已有选择不丢失。
...
| 自动名称 | 新计划自动按 Item rule 命名，不显示名称输入框；已有计划保存的名称继续保留。 |
...
| Item setup / Edit item setup | 集中查看本题的五项设置及只读难度要求；需要纠正 Domain、Context 或 Difficulty 时，暂存、查看影响并明确应用。 |
...
- 删除 Item rule 搜索输入、重复的标签页／跳转按钮、Save now、Save & validate、重复的预览标题和状态。
...
Submit for review 保存并检查题目后，会先让 AI 仅根据考生可见题面独立作答。这次调用不接收作者答案、评分、作者备注或翻译、语言目标及预设信息点。第二次调用才读取保存的答案、题目固定的 Assessment Settings，以及前一步保持不变的作答记录，进行核对。

AI preliminary review 展示 Independent answer、其他可能答案、原文证据和不确定之处。AI 与作者答案不同，只是供人工复核的线索，不会自动替换作者答案或证明题目错误；确认的缺陷仍沿用现有提交门槛。写作、口语可以有多种合理回答。图片和音频引用不代表模型已读取媒体，无法访问的材料会明确标为检查未完成。

独立作答与被检查的具体内容绑定，即使后续核对失败也保留。对已完成且仍匹配当前内容与规则的初审重试会复用记录；旧报告仍可查看，但新提交需要按当前流程重新初审。最终决定仍由人工审核作出。
